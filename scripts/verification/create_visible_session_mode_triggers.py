#!/usr/bin/env python3
"""Create visible cron trigger rows for manually checking session_mode in the UI.

Unlike verify_trigger_session_modes_http.py, this script intentionally leaves the
created agent/session/triggers in the real database so the triggers page can
...
async def remove_previous_demo(db: AsyncSession) -> None:
    ...
    trigger_ids = [
        row[0]
        for row in (await db.execute(select(JoySafeterTrigger.id).where(JoySafeterTrigger.name.like(f"{DEMO_NAME}-%")))).all()
    ]
    session_ids = []
    task_ids = []
    if agent_ids:
    ...
    if trigger_ids:
        task_ids.extend([row[0] for row in (await db.execute(select(JoySafeterTask.id).where(JoySafeterTask.trigger_id.in_(trigger_ids)))).all()])
...
async def main() -> None:
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            probe = curl_json("GET", "/triggers?type=cron&limit=1", RAW_API_KEY)
            print(f"auth ok existing_cron_trigger_probe={len(probe) if isinstance(probe, list) else 'unknown'}")
            ...
            agent = curl_json(
                ...
                RAW_API_KEY,
                {
                    ...
                    "engine_kind": "claude",
                    "model": "verify",
                    "system_prompt": "visible trigger session mode demo",
                    "description": "Demo agent for visible trigger session_mode rows",
            ...
            for mode in ("fresh", "reuse", "pinned"):
                body: dict[str, Any] = {
                    ...
                    "type": "cron",
                    "agent_id": agent_uuid,
                    "prompt_template": f"Visible trigger demo for {mode} session mode",
                    "cron_expr": "0 9 * * *",
                ...
                trigger = curl_json("POST", "/triggers", RAW_API_KEY, body)
                created.append(trigger)
                print(f"created trigger {trigger['name']} id={trigger['id']} session_mode={trigger.get('session_mode')}")

            print("VISIBLE_DEMO_READY open /managed/triggers and search: session-mode-visible-demo")
        finally:
