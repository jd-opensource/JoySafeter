...
#
# 用法:
#   ./tests/e2e_conductor_ha.sh
#   CONDUCTOR_URL=http://localhost:8080 ./tests/e2e_conductor_ha.sh
#   DB_URL="postgres://conductor:conductor@localhost:5432/conductor" ./tests/e2e_conductor_ha.sh
#
set -euo pipefail
...
cleanup() {
    ...
    db_query "UPDATE tasks SET status = 'failed', error = 'e2e cleanup', completed_at = NOW()
              WHERE status IN ('scheduling', 'pending') AND prompt LIKE '%E2E_CONDUCTOR_HA%'" > /dev/null 2>&1 || true
    echo -e "  Done."
}
trap cleanup EXIT

echo "╔═══════════════════════════════════════════════════╗"
echo "║  Conductor HA E2E Test                             ║"
echo "╚═══════════════════════════════════════════════════╝"

# ── Step 0: Preflight ────────────────────────────────────
info "Step 0: Preflight"

curl -sf "$API/health" > /dev/null && pass "Conductor reachable" || { fail "Conductor unreachable"; exit 1; }
docker exec "$PG_CONTAINER" psql -U conductor -d conductor -c "SELECT 1" > /dev/null 2>&1 && pass "PostgreSQL reachable via docker exec" || { fail "PostgreSQL unreachable"; exit 1; }
...
if [ -z "$AGENT_ID" ]; then
    ...
    AGENT_RESP=$(curl -sf "$API/agents" -X POST -H 'Content-Type: application/json' \
        -d '{
            "name": "e2e-conductor-ha-agent-'"$(date +%s)"'",
            "engine_kind": "claude",
...
db_query "INSERT INTO tasks (id, agent_id, status, prompt, output, timeout_sec, retry_count, max_retries, created_at)
          VALUES ('$TASK_UUID', '$AGENT_DB_ID', 'scheduling', 'E2E_CONDUCTOR_HA test scheduling CAS', '', 30, 0, 1, NOW())" > /dev/null
TASK_IDS+=("$TASK_UUID")
...
db_query "INSERT INTO tasks (id, agent_id, status, prompt, output, timeout_sec, retry_count, max_retries, created_at)
          VALUES ('$TASK_UUID2', '$AGENT_DB_ID', 'pending', 'E2E_CONDUCTOR_HA CAS pending test', '', 30, 0, 1, NOW())" > /dev/null
TASK_IDS+=("$TASK_UUID2")
...
db_query "INSERT INTO tasks (id, agent_id, status, prompt, output, timeout_sec, retry_count, max_retries, created_at)
          VALUES ('$STUCK_UUID', '$AGENT_DB_ID', 'scheduling', 'E2E_CONDUCTOR_HA stuck scheduling test', '', 30, 0, 1,
...
db_query "INSERT INTO tasks (id, agent_id, status, prompt, output, timeout_sec, retry_count, max_retries, created_at, started_at)
          VALUES ('$TIMEOUT_UUID', '$AGENT_DB_ID', 'running', 'E2E_CONDUCTOR_HA timeout test', '', 10, 0, 1,
...
# Try to acquire the recovery lock — if conductor is running, it should
...
if [ "$LOCK_RESULT" = "t" ]; then
    pass "Recovery advisory lock acquired (conductor not holding it post-startup)"
    # Release it
    db_query "SELECT pg_advisory_unlock(hashtext('task_recovery'))" > /dev/null
    pass "Recovery advisory lock released"
else
    # It's OK if conductor is currently running recovery — unlikely but valid
    pass "Recovery advisory lock held by conductor (recovery in progress)"
fi
...
if [ "$WDOG_LOCK" = "t" ]; then
    ...
    pass "Watchdog advisory lock held by conductor watchdog cycle"
fi
...
db_query "INSERT INTO tasks (id, agent_id, status, prompt, output, timeout_sec, retry_count, max_retries, created_at)
          VALUES ('$LIFECYCLE_UUID', '$AGENT_DB_ID', 'scheduling', 'E2E_CONDUCTOR_HA lifecycle test', '', 3600, 0, 2, NOW())" > /dev/null
...
db_query "INSERT INTO tasks (id, agent_id, status, prompt, output, timeout_sec, retry_count, max_retries, created_at, started_at)
          VALUES ('$RETRY_UUID', '$AGENT_DB_ID', 'running', 'E2E_CONDUCTOR_HA retry test', '', 3600, 0, 2, NOW(), NOW())" > /dev/null
TASK_IDS+=("$RETRY_UUID")
