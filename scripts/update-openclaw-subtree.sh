...
REMOTE_NAME="${OPENCLAW_REMOTE_NAME:-openclaw-upstream}"
REMOTE_URL="${OPENCLAW_REMOTE_URL:-https://github.com/openclaw/openclaw.git}"
PREFIX="backend/app/one_person_security_dept/openclaw"
REF="main"
SETUP_ONLY=0
DRY_RUN=0
SQUASH=1
...
usage() {
  cat <<'EOF'
Usage:
  update-openclaw-subtree.sh [options]
...
Environment variables:
  OPENCLAW_REMOTE_NAME   Remote name (default: openclaw-upstream)
  OPENCLAW_REMOTE_URL    Remote URL
...
Examples:
  ...
  ./scripts/update-openclaw-subtree.sh --ref main --dry-run
EOF
}
...
ensure_remote() {
  if git remote get-url "${REMOTE_NAME}" >/dev/null 2>&1; then
    ...
    if [[ "${current_url}" != "${REMOTE_URL}" ]]; then
      ...
      echo "Fix it manually or set OPENCLAW_REMOTE_NAME/OPENCLAW_REMOTE_URL." >&2
      exit 1
    fi
    return
  fi
