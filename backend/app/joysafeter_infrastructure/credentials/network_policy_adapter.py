...
class SqlAlchemyCredentialImpactAdapter:
    def __init__(self, db: object) -> None:
        ...
        self._advanced_session_ids: set[SessionId] = set()
    ...
    async def mark_pending(self, impact: CredentialImpact) -> CredentialImpact:
        ...
        if DependencyDisposition.REVALIDATE_ON_ACTIVATION in impact.dispositions:
            ...
            self._advanced_session_ids.update(runtime_session_ids)
            affected_session_ids.update(runtime_session_ids)
            affected_sandbox_ids.update(runtime_sandbox_ids)
        if DependencyDisposition.REFRESH_RUNTIME_POLICY in impact.dispositions:
            ...
            affected_sandbox_ids.update(network_sandbox_ids)
            self._pending.append(
                replace(
                    impact,
                    affected_sandbox_ids=frozenset(network_sandbox_ids),
                )
            )
        return replace(
            impact,
    ...
    async def nudge_after_commit(self) -> None:
        ...
        try:
            for impact in pending:
                if impact.source_id is None:
                    continue
                sandbox_ids = list(impact.affected_sandbox_ids)
