...
class RedisQueueBackend(QueueBackend):
    def __init__(self, redis_client, prefix: str = "conductor:queue"):
    ...
    async def push_global(self, task_id: uuid.UUID) -> None:
        key = f"{self._prefix}:global"
        try:
            await self._redis.lpush(key, str(task_id))
            self._redis_available = True
        except Exception:
    ...
    async def push_to_sandbox(self, sandbox_id: uuid.UUID, task_id: uuid.UUID) -> None:
        key = f"{self._prefix}:sandbox:{sandbox_id}"
        try:
            await self._redis.lpush(key, str(task_id))
        except Exception:
    ...
    async def pop_for_sandbox(
        ...
        try:
            while not cancel.is_set():
                brpop_task = asyncio.create_task(
                    self._redis.brpop(key, timeout=2)
                )
                cancel_task = asyncio.create_task(cancel.wait())
                done, pending = await asyncio.wait(
                    {brpop_task, cancel_task},
                ...
                if cancel_task in done:
                    return None
                if brpop_task in done:
                    result = brpop_task.result()
                    if result:
                        _, val = result
                        return uuid.UUID(val.decode() if isinstance(val, bytes) else val)
            return None
    ...
    async def drain_and_requeue_sandbox(self, sandbox_id: uuid.UUID) -> None:
        ...
        try:
            while True:
                ...
                if not val:
                    break
                await self._redis.lpush(global_key, val)
