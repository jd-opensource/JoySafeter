...
async def get_everos_memory_document(
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=5.0)) as client:
            ...
            visible_overview = _filter_overview_by_active_scopes(
                overview_response.json(),
                active_agent_ids=active_agent_ids,
                active_session_ids=active_session_ids,
            )
            if md_path not in _visible_md_paths(visible_overview):
                raise HTTPException(status_code=404, detail="EverOS memory document not found")
...
def _visible_md_paths(payload: dict[str, Any]) -> set[str]:
    paths: set[str] = set()
    for key in ("profiles", "episodes", "atomic_facts", "agent_cases", "agent_skills"):
        for item in _list_items(payload.get(key)):
