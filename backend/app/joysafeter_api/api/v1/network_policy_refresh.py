...
async def nudge_sandbox_network_policy_refreshes(
    ...
    *,
    ...
    """Best-effort post-commit acceleration for durable pending refreshes."""
    if not sandbox_ids:
        return 0

    semaphore = asyncio.Semaphore(10)

    async def _nudge(sandbox_id: SandboxId) -> bool:
    ...
    nudge_results = await asyncio.gather(*(_nudge(sandbox_id) for sandbox_id in sandbox_ids))
