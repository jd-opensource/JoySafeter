...
from app.joysafeter_domain.models.task import Task, TaskPriority
from app.joysafeter_domain.schemas import BaseResponse
from app.joysafeter_domain.schemas.task import (
    AssignTaskRequest,
    CreateTaskRequest,
    DispatchTaskRequest,
    TaskListResponse,
...
from app.joysafeter_api.services import TaskService
...
async def get_task(
    ...
    service = TaskService(db)
    task = await service.get_task(task_id, auth_ctx.project_id)
    if not task:
...
async def update_task(
    ...
    task = await service.update_task(task_id, auth_ctx.project_id, **updates)
    if not task:
        raise NotFoundError("Task not found", code="TASK_NOT_FOUND", data={"task_id": str(task_id)})
...
async def list_task_runs(
    ...
    from app.joysafeter_domain.schemas.agent_run import AgentRunResponse
    from app.joysafeter_api.services import AgentRunService

    service = AgentRunService(db)
    runs = await service.list_runs(project_id=auth_ctx.project_id, task_id=task_id)
    return BaseResponse(
        success=True,
        code=200,
        msg="ok",
