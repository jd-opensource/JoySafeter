...
pub(crate) struct DeliveryMetricsSnapshot {
    pub active_envoy_node_count: usize,
...
impl DeliveryCoordinator {
    ...
    pub(crate) fn metrics_snapshot(&self) -> DeliveryMetricsSnapshot {
        ...
        for pending in self.attempts.values() {
            ...
            let is_pending = pending.bound_key.as_ref().map_or(true, |key| {
                self.tracker.outcome(key) == Some(DeliveryOutcome::Pending)
            });
            if !is_pending {
                continue;
            }
        ...
        DeliveryMetricsSnapshot {
            active_envoy_node_count: self.node_sessions.len(),
