...
impl XdsAuthKeyring {
    pub fn parse(raw: &str, write_key_id: &str) -> anyhow::Result<Self> {
        ...
        for (key_id, token) in values {
            ...
            if !(MIN_TOKEN_BYTES..=MAX_TOKEN_BYTES).contains(&token.as_bytes().len()) {
                anyhow::bail!(
        ...
        let write_key_index = write_key_index.ok_or_else(|| {
            anyhow::anyhow!(
    ...
    pub fn write_token(&self) -> &str {
        &self.keys[self.write_key_index].token
    }

    pub fn write_token_matches(&self, token: &str) -> bool {
        let candidate: [u8; 32] = Sha256::digest(token.as_bytes()).into();
        bool::from(candidate.ct_eq(&self.keys[self.write_key_index].digest))
    }
}
