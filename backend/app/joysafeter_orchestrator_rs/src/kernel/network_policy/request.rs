...
impl NetworkPolicyRequest {
    ...
    pub fn remove(sandbox_id: SandboxId, generation: NetworkPolicyGeneration) -> Self {
        Self {
            sandbox_id,
...
mod tests {
    ...
    #[test]
    fn removal_request_carries_the_generation_being_retired() {
        let sandbox_id = SandboxId::from_uuid(Uuid::now_v7());
        let generation = NetworkPolicyGeneration {
            policy_hash: "policy-hash".to_string(),
            policy_version: 7,
        ...
        let request = NetworkPolicyRequest::remove(sandbox_id, generation.clone());
