...
use crate::ids::{MemoryId, MemoryStoreId, MemoryVersionId, SandboxId, SessionId, TaskId};
...
use crate::kernel::runner::execution::{
    multi_task_loop, RunnerExecutionService, TaskMessageOutcome, TaskResult,
};
use crate::kernel::runner::recovery::{ReconnectPlan, RunnerRecoveryService};
...
impl RunnerSessionCoordinator {
    pub(crate) fn new(
        ...
        let execution = RunnerExecutionService::new(config.grpc_max_executions);
        Self {
            bridge_store,
            event_bus,
            queue,
            ...
            runtime_config,
            execution,
            recovery: RunnerRecoveryService::new(),
            redis_coordinator,
            memory_subscribers,
        }
    }
...
impl RunnerSessionCoordinator {
    pub(crate) async fn open_session(
        ...
        let execution_semaphore = self.execution.semaphore();
        let recovery_service = self.recovery.clone();
