...
pub async fn extract_archive_to_dir(
    archive_path: PathBuf,
    target_dir: PathBuf,
) -> anyhow::Result<()> {
    tokio::task::spawn_blocking(move || extract_archive_to_dir_blocking(&archive_path, &target_dir))
        .await?
}

pub fn build_tar_from_dir(root: &Path) -> anyhow::Result<Vec<u8>> {
    let mut tar_buf = Vec::new();
    {
        let mut builder = tar::Builder::new(&mut tar_buf);
        append_dir_contents(&mut builder, root, root)?;
        builder.finish()?;
    }
    Ok(tar_buf)
}
