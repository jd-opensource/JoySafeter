...
from app.joysafeter_shared.tools.tool import EnhancedTool, ToolFilter, ToolMetadata, ToolSourceType
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # User/Project ownership index (for fast query of tools owned by user)
        self._owner_user_index: Dict[str, Set[str]] = {}
        self._owner_project_index: Dict[str, Set[str]] = {}
    ...
    def register_mcp_tools(
        ...
        Args:
            ...
            owner_project_id: Project ID (optional)
    ...
    def get_tools_for_scope(
        ...
        """
        Get tools available in user/project scope

        Includes:
        - Builtin tools (builtin) - if include_builtin=True
        - Tools owned by user (owner_user_id == user_id, owner_project_id is None)
        - Project level tools (owner_project_id == project_id) - if project_id provided
    ...
    def _update_indexes(self, tool_name: str, tool_metadata: ToolMetadata):
        ...
        if tool_metadata.owner_project_id:
            if tool_metadata.owner_project_id not in self._owner_project_index:
                self._owner_project_index[tool_metadata.owner_project_id] = set()
            self._owner_project_index[tool_metadata.owner_project_id].add(tool_name)
    ...
    def _remove_from_indexes(self, tool_name: str, tool_metadata: ToolMetadata):
        ...
        # Project ownership index
        if tool_metadata.owner_project_id and tool_metadata.owner_project_id in self._owner_project_index:
            self._owner_project_index[tool_metadata.owner_project_id].discard(tool_name)
...
def _initialize_builtin_tools(registry: ToolRegistry):
    ...
    try:
        from app.joysafeter_shared.tools.builtin.research_tools import tavily_search, think_tool
        ...
        # --- Skill Preview ---
        from app.joysafeter_shared.tools.builtin.preview_skill import preview_skill_in_sandbox
