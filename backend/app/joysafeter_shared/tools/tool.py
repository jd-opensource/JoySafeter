...
class ToolMetadata:
    ...
    owner_project_id: Optional[str] = None  # Project scope (NULL = user-level, global)
...
class ToolFilter:
    ...
    owner_project_id: Optional[str] = None  # Filter by project scope
    ...
    def _matches_ownership(self, metadata: ToolMetadata) -> bool:
        ...
        - User-level tools (owner_project_id=None): private tools, visible only to owner
        - Project-level tools (owner_project_id!=None): shared tools, visible to all project members
        """
        # If no owner filter specified, match all
        if self.owner_user_id is None and self.owner_project_id is None:
            return True
        ...
        # Project-level tools: match by project_id (any user's tools in the project)
        if metadata.owner_project_id is not None:
            if self.owner_project_id is not None:
                return metadata.owner_project_id == self.owner_project_id
            # If no project filter, project-level tools are not visible
            return False
