"""
Conductor Kernel Lifespan (startup/shutdown).

This module provides conductor_startup() and conductor_shutdown() functions
for integrating the Conductor kernel into the application lifecycle.
"""
...
from app.core.settings import ConductorConfig, conductor_config
...
def _create_sandbox_provider():
    from app.core.sandbox.docker_provider import DockerSandboxProvider

    provider_name = conductor_config.sandbox_provider
    if provider_name == "daytona":
        from app.core.sandbox.daytona_provider import DaytonaSandboxProvider
        ...
        from app.core.sandbox.e2b_provider import E2bSandboxProvider
...
async def conductor_startup() -> None:
    ...
    if not conductor_config.enabled:
        logger.info("Conductor kernel disabled")
        return

    logger.info("Starting Conductor kernel...")

    from app.core.queue import InMemoryRedisQueueBackend
    from app.core.scheduler import TaskScheduler
    from app.core.task_controller import TaskController
    from app.core.sandbox_controller import SandboxController
    from app.core.session_broadcaster import SessionBroadcaster
    from app.core.sandbox_bridge import SandboxBridgeRegistry
    from app.core.events.event_buffer import EventBatchSender, EventBatchConfig
    from app.core.sandbox_resolver import SandboxResolver
    from app.core.memory_sync import MemoryStoreSubscribers
    from app.core.runtime.registry import AdapterRegistry
    ...
    if redis_client:
        from app.core.redis_coordinator import RedisCoordinator
    ...
    if conductor_config.envoy_enabled:
        try:
            from app.core.sandbox.envoy_manager import EnvoyManager, EnvoyConfig
    ...
    if conductor_config.image_builder_enabled:
        try:
            from app.core.sandbox.image_builder import ImageBuilder
    ...
    if conductor_config.vault_encryption_key:
        from app.services.vault_cipher import VaultCipher
    ...
    from app.core.events.bus import ConductorEventBus
    from app.core.events.subscribers.event_persist import EventPersistSubscriber
    from app.core.events.subscribers.session_state import SessionStateSubscriber
    from app.core.events.subscribers.session_broadcast import SessionBroadcastSubscriber
    from app.core.events.subscribers.task_broadcast import TaskBroadcastSubscriber

    _event_bus = ConductorEventBus()
    ...
    logger.info("   ConductorEventBus initialized (4 subscribers)")

    # gRPC server for runner connections
    try:
        from app.core.grpc.server import start_grpc_server
    ...
    if _redis_coordinator and redis_client:
        from app.core.command_listener import CommandListener
