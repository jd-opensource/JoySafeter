...
from app.everos.core.errors import InvalidInputError, NotFoundError
...
class ReplayResponse(BaseModel):
    """Response body for dead-letter replay."""
...
@router.get("/dead_letters", response_model=list[RunRecord])
async def list_dead_letters(
    ...
    """List OME runs that exhausted retries and reached DEAD_LETTER."""
    from app.everos.service.memorize import _get_engine  # noqa: SLF001

    engine = _get_engine()
    records = await engine.list_dead_letters(
...
@router.post("/dead_letters/{run_id}/replay", response_model=ReplayResponse)
async def replay_dead_letter(
    run_id: str,
    timeout: float = Query(default=120.0, gt=0),
) -> ReplayResponse:
    """Replay one DEAD_LETTER run with its original event payload."""
    from app.everos.service.memorize import _get_engine  # noqa: SLF001

    engine = _get_engine()
    try:
        replay_run_id = await engine.replay_dead_letter(run_id)
    except KeyError:
        raise NotFoundError(f"run '{run_id}' not found") from None
    except ValueError as exc:
        raise InvalidInputError(str(exc)) from exc
    logger.info(
        "ome_dead_letter_replay",
        run_id=run_id,
        replay_run_id=replay_run_id,
    )
    idle = await engine.wait_idle(timeout=timeout)
    if not idle:
        logger.warning(
            "ome_dead_letter_replay_timeout",
            run_id=run_id,
            replay_run_id=replay_run_id,
            timeout=timeout,
        )
    ...
    return ReplayResponse(status="ok", run_id=run_id, replay_run_id=replay_run_id)
