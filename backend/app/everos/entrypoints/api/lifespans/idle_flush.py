...
def _resolve_interval_and_threshold() -> tuple[float, int]:
    memorize = load_settings().memorize
    return (
        float(memorize.idle_flush_scan_interval_seconds),
        int(memorize.idle_flush_threshold_seconds),
    )


class IdleFlushLifespanProvider(LifespanProvider):
    """Run a periodic idle-buffer flush loop while the API app is up."""
