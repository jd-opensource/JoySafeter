...
from app.everos.core.persistence.markdown import dump_frontmatter, parse_frontmatter
...
if TYPE_CHECKING:
    ...
    from app.everos.config.settings import KnowledgeSearchSettings
    from app.everos.memory.search.recall import KnowledgeTopicRecaller
...
def _get_embedding() -> EmbeddingProvider | None:
    ...
    from app.everos.component.embedding import (  # Deferred: singleton
        build_embedding_provider,
    )
    from app.everos.config import load_settings  # Deferred: singleton
...
def _build_recaller() -> KnowledgeTopicRecaller:
    ...
    from app.everos.component.tokenizer import (  # Deferred: singleton
        build_tokenizer,
    )
    from app.everos.memory.search.recall import (  # Deferred: singleton
...
def _get_reranker() -> RerankProvider | None:
    ...
    from app.everos.component.rerank import (  # Deferred: singleton
...
async def _run_category_pipeline(
    ...
    *,
    ...
    from app.everos.memory.search.callbacks import (  # Deferred: heavy dep
        build_rerank_fn,
    )
