...
fn build_http_listener_proto(
    ...
    let http_filters = vec![HttpFilter {
    ...
    let hcm = HttpConnectionManager {
        ...
        stream_idle_timeout: Some(envoy_types::pb::google::protobuf::Duration {
            seconds: 0,
            nanos: 0,
        }),
...
pub(crate) fn build_virtual_hosts_proto(
    ...
    for (match_host, routes) in group_route_specs_by_host(credentials) {
        let proto_routes: Vec<Route> = routes
            ...
            .map(|r| {
                ...
                let request_headers_to_add = r
                    .inject_headers
                    .iter()
                    .map(|(name, value)| HeaderValueOption {
                        header: Some(HeaderValue {
                            key: name.clone(),
                            value: escape_envoy_header_value(value),
                            ..Default::default()
                        }),
                        append_action:
                ...
                Route {
                    ...
                    request_headers_to_remove: upstream_headers_to_remove(r),
