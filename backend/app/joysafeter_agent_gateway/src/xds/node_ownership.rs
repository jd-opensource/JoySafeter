...
impl NodeOwnershipRegistry {
    ...
    pub fn assigned_node_names(&self) -> HashSet<String> {
        self.inner
            .assignments
            .read()
            .expect("node ownership registry poisoned")
    ...
    pub async fn wait_for_delivery_owner_node(
        ...
        if self.inner.scope == VisibilityScope::Unscoped {
            return Ok(None);
        }

        let mut revision = self.subscribe();
        loop {
            if let Some(node) = self.owner_node(sandbox_id) {
                return Ok(Some(node));
            }
            revision.changed().await.map_err(|_| {
                anyhow::anyhow!(
                    "xDS node ownership registry closed while waiting for sandbox {sandbox_id}"
                )
            })?;
        }
    }
...
#[cfg(test)]
#[path = "../../tests/unit/xds/node_ownership_test.rs"]
mod tests;
