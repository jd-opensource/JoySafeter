...
from app.joysafeter_domain.agent.backends.constants import DOCKER_UNAVAILABLE_MSG
from app.joysafeter_domain.agent.backends.docker_check import is_docker_available
from app.joysafeter_domain.graph.deep_agents.agent_factory import (
    build_a2a_worker,
    build_code_agent_worker,
    build_standard_worker,
)
from app.joysafeter_domain.graph.deep_agents.config import NodeConfig, resolve_all_configs
from app.joysafeter_domain.graph.deep_agents.middleware import resolve_memory_middleware
from app.joysafeter_domain.graph.deep_agents.model_resolver import ModelResolver
from app.joysafeter_domain.graph.deep_agents.skills_loader import (
    has_valid_skills,
    preload_skills,
    resolve_skill_ids,
)
from app.joysafeter_domain.graph.deep_agents.tool_resolver import resolve_tools
from app.joysafeter_domain.graph.runtime_prompt_template import build_runtime_prompt_context, render_runtime_template
...
async def build_deep_agents_graph(
    ...
    if (hard_docker or soft_docker) and user_id:
        ...
        if docker_ok:
            ...
            if backend and file_emitter:
                from app.joysafeter_domain.agent.backends.file_tracking_proxy import FileTrackingProxy
    ...
    try:
        ...
        from app.joysafeter_domain.agent.checkpointer.checkpointer import get_checkpointer
...
def _finalize(agent: Any, backend: Any, sandbox_handle: Any = None) -> Any:
    ...
    if backend:
        from app.joysafeter_domain.agent.backends.pydantic_adapter import PydanticSandboxAdapter
...
async def _get_user_sandbox(user_id: Any, sandbox_port: Any = None) -> Any:
    ...
    if sandbox_port is None:
        from app.joysafeter_domain.services.sandbox_manager import get_sandbox_handle
