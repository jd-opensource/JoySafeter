...
from app.joysafeter_shared.common.app_errors import InvalidRequestError, ServiceUnavailableError
from app.joysafeter_shared.tools.tool import EnhancedTool, ToolFilter, ToolSourceType
from app.joysafeter_shared.tools.tool_registry import ToolRegistry, get_global_registry
from app.joysafeter_domain.models.enums import McpConnectionStatus
from app.joysafeter_domain.models.mcp import McpServer
from app.joysafeter_domain.schemas.mcp import (
...
from app.joysafeter_domain.services.mcp_client_service import McpClientService, get_mcp_client
from app.joysafeter_domain.services.mcp_server_service import McpServerService
...
class ToolService(BaseService[McpServer]):
    ...
    def get_available_tools(
        ...
        tools = self._registry.get_tools_for_scope(
            user_id=user_id,
            project_id=None,  # user-level, no project
    ...
    async def _sync_server_tools(self, server: McpServer) -> List[ToolInfo]:
        ...
        try:
            ...
            registered = self._registry.register_mcp_tools(
                ...
                owner_project_id=None,  # user-level, no project
                category="mcp",
            )
    ...
    async def _unregister_server_tools(self, server: McpServer) -> int:
        ...
        from app.joysafeter_domain.services.mcp_toolkit_manager import get_toolkit_manager
    ...
    def _tool_to_info(self, tool: EnhancedTool) -> ToolInfo:
        ...
        return ToolInfo(
            ...
            owner_project_id=None,  # user-level, no longer uses project
