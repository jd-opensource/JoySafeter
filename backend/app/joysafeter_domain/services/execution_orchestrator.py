...
from app.joysafeter_shared.common.app_errors import InvalidRequestError, NotFoundError, normalize_app_error
from app.joysafeter_domain.constants import RunPurpose, TriggerMedium
from app.joysafeter_domain.contracts.execution import ACTIVE_RUN_STATUSES
...
class ExecutionOrchestrator:
    ...
    async def dispatch_draft(
        ...
        if agent.project_id != project_id:
            raise InvalidRequestError(
                ...
                data={"agent_id": str(agent_id), "project_id": project_id},
            )
    ...
    async def retry_run(self, run_id: uuid.UUID, user_id: str) -> AgentRun:
        ...
        from app.joysafeter_domain.services.execution_launcher import ExecutionLauncher, LaunchSpec
        ...
        try:
            ...
            await self._publish_launch_failure(execution, run.id, run.project_id or "", exc)
    ...
    async def _create_run_and_fire(self, spec: RunSpec) -> AgentRun:
        ...
        try:
            ...
            await self._publish_launch_failure(execution, run.id, spec.project_id, exc)
            await self.db.refresh(run)
