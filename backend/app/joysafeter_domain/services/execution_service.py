...
from app.joysafeter_shared.common.app_errors import InternalServiceError, NotFoundError
...
from app.joysafeter_domain.ports.execution import EventContext
from app.joysafeter_domain.models.execution import (
    Execution,
    ExecutionEvent,
)
from app.joysafeter_domain.repositories.execution import ExecutionEventRepository, ExecutionRepository
    ...
    async def mark_status(
        ...
        *,
        ...
        if ctx is None:
            ...
            result = await self.db.execute(
                select(Execution, AgentRun.project_id)
        ...
        envelope = ExecutionEventEnvelope(
            ...
            trigger_medium=ctx.trigger_medium,
            run_purpose=ctx.run_purpose,
    ...
    async def append_event(
        ...
        *,
        ...
        envelope = ExecutionEventEnvelope(
            ...
            project_id=self._event_ctx.project_id,
            ...
            for execution in stale:
                try:
                    ...
                    envelope = ExecutionEventEnvelope(
                        ...
                        error=stale_error,
                        result_summary="Reaped: stale execution",
                        trigger_medium=run.trigger_medium if run else None,
                        run_purpose=run.run_purpose if run else None,
