...
from app.joysafeter_domain.agent.base_agent import get_agent
from app.joysafeter_domain.copilot.prompt_builder import build_copilot_system_prompt
from app.joysafeter_domain.copilot.tools import get_copilot_tools, set_current_graph_context, set_preloaded_models
...
async def _preload_available_models(db: AsyncSession) -> List[Dict[str, Any]]:
    ...
    """
    try:
        from app.joysafeter_shared.model import ModelType
...
async def get_copilot_agent(
    ...
    Args:
        ...
        user_id: Optional user ID for sandbox isolation
