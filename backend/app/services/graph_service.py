...
class GraphService(BaseService):
    ...
    async def create_skill_creator_graph(
        ...
        """
        Build a Skill Creator Agent graph in memory (no DB persistence).
        Used for skill creation/editing conversations.

        Constructs one root node with useDeepAgents=true, a skill-creator system
        prompt, and no edges, then uses GraphBuilder so that
        DeepAgentsGraphBuilder builds a standalone create_deep_agent graph.

        Args:
            edit_skill_id: If provided, the agent enters editing mode for the
                           given skill ID instead of creating a new skill.
        ...
        # Single root node with DeepAgents enabled and skill-creator config
        node = GraphNode(
            id=node_id,
            graph_id=graph_id,
            type="agent",
        ...
        logger.info(
            f"[GraphService] ===== create_skill_creator_graph START ===== | "
            f"user_id={user_id} | llm_model={llm_model} | edit_skill_id={edit_skill_id}"
        )
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(
            f"[GraphService] ===== create_skill_creator_graph COMPLETE ===== | "
            f"user_id={user_id} | elapsed={elapsed_ms:.2f}ms | edit_skill_id={edit_skill_id}"
        )
        return compiled_graph
