...
# ---------------------------------------------------------------------------
# JoySafeter event bus (separate domain from ExecutionEventBus)
# ---------------------------------------------------------------------------

from app.joysafeter_worker.events.envelope import JoySafeterEventEnvelope
from app.joysafeter_worker.events.subscriber import JoySafeterEventSubscriber
...
class JoySafeterEventBus:
    ...
    def register(self, sub: JoySafeterEventSubscriber) -> None:
        ...
        if sub.phase == SubscriberPhase.PERSIST:
            ...
            self._broadcast_subs.append(sub)
        logger.info("Registered joysafeter event subscriber: %s (phase=%s)", sub.name, sub.phase.name)
