...
from app.core.graph.graph_schema import EdgeSchema, GraphSchema, NodeSchema, StateFieldSchema
...
async def test_global_error_policy():
    ...
    class MockBuilder:
        def __init__(self):
            ...
            self.nodes = [MockNode("node_error"), MockNode("node_fallback")]
...
async def test_global_error_validation():
    ...
    with pytest.raises(ValidationError, match="Fallback node ID 'missing_node' not found"):
        GraphSchema(nodes=[NodeSchema(id="n1", type="test")], fallback_node_id="missing_node")
