    ...
    async with SessionLocal() as db:
        ...
        try:
            ...
            global_cred = ModelCredential(
                ...
                credentials="global_encrypted_stuff",
                is_valid=True
            )
            db.add(global_cred)
            await db.commit()
            ...
            # Cleanup
            await db.execute(delete(ModelCredential).where(ModelCredential.provider_name.in_([provider_name, global_provider])))
            await db.commit()
