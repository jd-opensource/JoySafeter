# Runtime images using a prebuilt Linux AMD64 runner.
...
RUN arch="$(dpkg --print-architecture)" \
    ...
    && apt-get install -y --no-install-recommends \
        ...
        git curl wget jq tar zip unzip openssh-client tmux screen \
        make cmake ripgrep tree htop vim nano sqlite3 postgresql-client redis-tools \
