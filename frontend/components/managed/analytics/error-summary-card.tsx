...
export function ErrorSummaryCard({ data, loading }: ErrorSummaryCardProps) {
  ...
  if (!data || data.total_errors === 0) {
    return (
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <p className="text-xs text-muted-foreground">{t('analytics.errorSummary.noErrors')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-medium text-foreground">{t('analytics.errorSummary.title')}</h3>
      ...
      <div className="mb-3 flex h-3 gap-px overflow-hidden rounded-full">
