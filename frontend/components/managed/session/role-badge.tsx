...
const roleStyles: Record<string, { bg: string; labelKey: string }> = {
  ...
  "session.status_idle": { bg: "bg-gray-400 text-gray-800 dark:bg-gray-500 dark:text-gray-100", labelKey: "idle" },
  ...
  "session.thread_status_idle": { bg: "bg-gray-400 text-gray-800 dark:bg-gray-500 dark:text-gray-100", labelKey: "idle" },
...
export function RoleBadge({ eventType }: { eventType: string; toolName?: string }) {
  ...
  return (
    <span className={cn("shrink-0 inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium min-w-[52px] justify-center", style?.bg || "bg-gray-500 text-white")}>
      {label}
    </span>
  )
}
