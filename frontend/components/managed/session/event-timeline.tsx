...
export function EventTimeline({ events, sessionStart, selectedId, onSelect }: EventTimelineProps) {
  ...
  return (
    <div className="relative">
      <div
        ref={containerRef}
        className="relative h-8 cursor-pointer overflow-hidden rounded-md border border-border bg-secondary dark:bg-[#1e1e1e]"
        onClick={handleClick}
      ...
      >
        {events.map((evt, i) => {
          ...
          return (
            <div
              key={evt.id || i}
              className="absolute bottom-[4px] top-[4px] rounded-sm"
              style={{
                left: `${positions[i]}%`,
      ...
      {hoverIndex !== null && events[hoverIndex] && (
        <div
          className="pointer-events-none absolute -top-7 z-20 whitespace-nowrap rounded border border-border bg-popover px-1.5 py-0.5 text-[10px] text-popover-foreground shadow-sm"
          style={{
...
function getEventColor(type: string): string {
  // User events — red
  if (type === 'user.message' || type === 'user.interrupt' || type === 'user.define_outcome')
    return '#ef4444'
  ...
  if (
    type === 'agent.bg_task_started' ||
    type === 'agent.bg_task_progress' ||
    type === 'agent.bg_task_finished'
  )
    return '#6366f1'
