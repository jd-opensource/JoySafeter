...
import { InterruptPanel } from '@/components/editors/graph-builder/components/InterruptPanel'
            ...
            {isSearching && (
              <div className="flex h-8 shrink-0 items-center gap-2 border-b border-[var(--border)] bg-[var(--surface-elevated)] px-3">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent font-mono text-sm text-[var(--text-secondary)] outline-none placeholder:text-[var(--text-subtle)]"
                  onKeyDown={(e) => {
                    if (e.key === 'Escape') {
                      setIsSearching(false)
                      setSearchQuery('')
