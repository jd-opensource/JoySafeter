...
export function normalizeObservation(raw: Record<string, unknown>): RawObservation {
  ...
  return {
    ...
    parentObservationId: (raw.parent_observation_id ?? raw.parentObservationId ?? null) as
      | string
      | null,
    ...
    completionStartTime: (raw.completion_start_time ?? raw.completionStartTime ?? null) as
      | string
      | null,
    ...
    modelParameters: (raw.model_parameters ?? raw.modelParameters ?? null) as Record<
      string,
      unknown
    > | null,
    ...
    calculatedOutputCost: (raw.calculatedOutputCost ?? costDetails?.output ?? null) as
      | number
      | null,
