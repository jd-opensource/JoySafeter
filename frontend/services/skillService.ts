  ...
  if (frontmatter.metadata && typeof frontmatter.metadata !== 'object') {
    try {
      frontmatter.metadata =
        typeof frontmatter.metadata === 'string' ? JSON.parse(frontmatter.metadata) : {}
    } catch {
      frontmatter.metadata = {}
    }
  }
...
function toBackendSkill(skill: Partial<Skill>): any {
  ...
  return {
    ...
    source_type:
      skill.source_type ||
      (skill.source === 'git' ? 'git' : skill.source === 's3' ? 's3' : 'local'),
...
export const skillService = {
  ...
  async saveSkill(
    skill: Partial<Omit<Skill, 'id' | 'updated_at' | 'created_at'>> & { id?: string; name: string },
  ): Promise<Skill> {
    try {
