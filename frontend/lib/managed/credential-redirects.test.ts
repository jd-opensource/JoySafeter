...
describe('credential redirect helpers', () => {
  it('maps secrets list + create params', () => {
    ...
    expect(resolveSecretsRedirect('generic')).toBe(
      '/managed/credentials?tab=services&create=service',
    )
    expect(resolveSecretsRedirect('custom')).toBe(
      '/managed/credentials?tab=services&create=service',
    )
  ...
  it('maps vaults list + create param', () => {
    ...
    expect(resolveVaultsRedirect('1')).toBe('/managed/credentials?tab=mcp&create=credential-group')
  })
})
