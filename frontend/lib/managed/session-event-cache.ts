...
export function setCachedSessionEventState(
  ...
  const droppedOlderEvents = sorted.length > MAX_CACHED_EVENTS_PER_SESSION
  const normalized = droppedOlderEvents
    ? sorted.slice(sorted.length - MAX_CACHED_EVENTS_PER_SESSION)
    : sorted
  const state: CachedSessionEventState = {
    events: normalized,
    hasMoreOlder: hasMoreOlder || droppedOlderEvents,
