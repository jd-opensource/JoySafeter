...
function ActivityList({
  ...
  onOpenDetail,
  emptyTitle = '暂无近期记忆活动',
  emptyDescription = 'EverOS 写入记忆后会在这里形成时间线。',
}: {
...
function EpisodeTimeline({
  ...
  if (!episodes.length) {
    return <EmptyState title="暂无 Episode" description="沙箱会话产生可沉淀内容后会写入 EverOS episode 文件。" />
  }
...
function getDreamingStatusMessage({
  mutationError,
  mutationIsError,
  mutationIsSuccess,
  runError,
  ...
  runError: Error | null
  runIsError: boolean
  runStatus?: DreamingRunStatus
}) {
  if (mutationIsError) return mutationError?.message || 'Dreaming failed'
  if (runIsError) return runError?.message || 'Dreaming status failed'
  if (runStatus?.status === 'running') return 'Dreaming running'
  if (runStatus?.status === 'success') return 'Dreaming complete'
  if (runStatus?.status === 'failed') return runStatus.error || 'Dreaming failed'
  if (runStatus?.status === 'dead_letter') return runStatus.error || 'Dreaming moved to dead letter'
  if (runStatus?.status === 'crashed') return runStatus.error || 'Dreaming crashed'
  if (mutationIsSuccess) return 'Dreaming queued'
  return null
}
...
export default function MemoryStoreListPage() {
  ...
  const dreamingFailed = ['failed', 'dead_letter', 'crashed'].includes(displayedDreamingRunState || '')
  const dreamingButtonLabel = dreamingIsRunning
    ? 'Dreaming run...'
    : displayedDreamingRunState === 'success'
      ? 'Dreaming Complete'
      : 'Dreaming'
  const dreamingStatusMessage = getDreamingStatusMessage({
    mutationError: dreamingMutation.error,
    mutationIsError: dreamingMutation.isError,
    mutationIsSuccess: dreamingMutation.isSuccess,
    runError: dreamingRunStatusError,
    runIsError: isDreamingRunStatusError,
    runStatus: displayedDreamingRunStatus,
  })
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="记忆存储"
        subtitle="查看 EverOS 自动沉淀的用户记忆与智能体记忆。"
        action={
              ...
              <div className="grid gap-4 xl:grid-cols-[minmax(280px,420px)_1fr]">
                <section data-testid="profile-memory-section" className="space-y-3">
                  ...
                  {visibleProfiles.length ? (
                    ...
                    <EmptyState title="暂无用户画像" description="EverOS 提取 profile 后会展示显式信息和隐式特征。" />
                  )}
                </section>
                <section data-testid="episode-memory-section" className="space-y-3">
                  ...
                  {dreamingStatusMessage && (dreamingFailed || dreamingMutation.isError || isDreamingRunStatusError) ? (
                    <p className={cn('text-xs', dreamingFailed || dreamingMutation.isError || isDreamingRunStatusError ? 'text-destructive' : 'text-emerald-600')}>
                      {dreamingStatusMessage}
                    </p>
                  ) : null}
                  <EpisodeTimeline
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                ...
                <section data-testid="agent-skill-section" className="space-y-3">
                  ...
                  {visibleAgentSkills.length ? (
                    ...
                    <EmptyState title="暂无 Agent Skill" description="EverOS 从高质量案例归纳技能后会展示在这里。" />
                  )}
                </section>
              </div>
            </TabsContent>
