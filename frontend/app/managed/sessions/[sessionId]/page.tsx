...
export default function SessionDetailPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const { data: vaultCredentials } = useQuery({
    ...
    queryFn: () =>
      managedGet<{ data: VaultCredential[] }>(
        `/vaults/${stripIdPrefix(vaultId!)}/credentials?limit=100`,
      ),
  ...
  const { data: sessionResources } = useQuery({
    ...
    queryFn: () =>
      managedGet<{ data: SessionResource[] }>(`/sessions/${stripIdPrefix(id)}/resources`),
    enabled: !!id,
  })
                  ...
                  {isIdle && (
                    <div className="flex items-center gap-1.5 pt-1.5">
                      <Input
                        ...
                        onChange={(e) =>
                          setTokenEdits((prev) => ({ ...prev, [r.id]: e.target.value }))
                        }
