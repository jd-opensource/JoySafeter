...
export function CreateSessionDialog({ open, onOpenChange, onCreated }: CreateSessionDialogProps) {
  ...
  const filteredAvailableMemoryStores = useMemo(() => {
    ...
    return availableMemoryStores.filter((store) =>
      `${store.name} ${store.id}`.toLowerCase().includes(q),
    )
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      const isInteractiveTarget = Boolean(
        element?.closest(
          'button,input,textarea,select,a,[role="option"],[data-dropdown-interactive="true"]',
        ),
      )
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="space-y-4 py-4">
          <FormSectionCard
            ...
            description={t(
              'managed.sessions.create.basicSettingsDesc',
              '选择要运行的智能体，并可设置本次会话标题。',
            )}
          >
            {/* Title */}
            <div>
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                <FormFieldLabel required>{t('managed.sessions.create.agent')}</FormFieldLabel>
                <button
                ...
                <div className="mt-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="shrink-0 rounded bg-background px-1.5 py-0.5 font-mono uppercase">
                      {selectedAgent.engine_kind ||
                        t('managed.sessions.create.unknownEngine', 'unknown')}
                    </span>
                  </div>
                  <div className="mt-1 truncate">
                    {selectedAgentDefaultEnv
                      ...
                      : t(
                          'managed.sessions.create.selectedAgentNoEnvironment',
                          '未配置默认运行环境',
                        )}
                  </div>
