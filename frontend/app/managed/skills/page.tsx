...
interface MoveVariables extends SkillActionScope {
  source: SkillWorkspaceMoveSource<SkillFileId>
  destFolder: string
  ...
  const moveMutation = useMutation({
    mutationFn: async (variables: MoveVariables) => {
      ...
      if (source.kind === 'file') {
        ...
        if (files.some((f) => f.path === dest && f.id === source.fileKey)) return
        // Conflict: a different file with the same name already sits in dest.
        const movingFile = files.find((f) => f.id === source.fileKey)
        if (
          movingFile &&
          files.some(
            (f) =>
              f.id !== source.fileKey && f.path === dest && f.file_name === movingFile.file_name,
          )
        ) {
        ...
        parseSkillFileResponse(
          await managedPut<unknown>(
            apiResourcePath('skills', skillId, 'files', apiResourceId(source.fileKey)),
            { path: dest },
            {
              ...managedRequestOptions(variables.scope),
              timeout: SKILL_SCAN_TIMEOUT_MS,
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden rounded-xl border border-border bg-background">
        ...
        <SkillWorkspace
          ...
          onMove={(source, destFolder) => {
            ...
            if (source.kind === 'file') {
              const file = currentSkillFile(source.fileKey, action.skillId)
              if (!file) return
              moveMutation.mutate({
                ...action,
