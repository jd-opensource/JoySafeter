...
import { useSkillAuthoring, type SkillDraft, type SkillDraftFile } from '@/hooks/managed/use-skill-authoring'
...
const VISIBILITY_OPTIONS: Array<{ value: NonNullable<SkillDraft['visibility']>; labelKey: string }> = [
...
export default function SkillAiAuthoringPage() {
  ...
  const onUploadChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const TEXT_EXT = /\.(md|markdown|txt|json|ya?ml|py|js|ts|tsx|jsx|sh|toml|cfg|ini|csv|html?|xml|env)$/i

    // Strip a common leading directory (webkitdirectory prefixes every path
    // with the picked folder's name).
    const rels = list.map((f) => (f as File & { webkitRelativePath?: string }).webkitRelativePath || f.name)
  ...
  const onPublish = async () => {
    ...
    if (!skillId) {
      toast({ title: error || t('managed.skills.aiAuthor.errors.saveFailed'), variant: 'destructive' })
      return
    }
    if (error) {
  ...
  return (
    <div className="flex h-screen flex-col bg-background -m-5">
      ...
      <div className="flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span title={!draftSkillId ? t('managed.skills.aiAuthor.scan.saveFirst') : undefined}>
            <Button variant="outline" size="sm" onClick={runScan} disabled={!draftSkillId || scanRunning}>
          ...
          <Button variant="outline" size="sm" onClick={saveDraft} disabled={streaming || publishing || !draft.name.trim()}>
            <Save className="mr-1 h-3.5 w-3.5" />
            {t('managed.skills.aiAuthor.saveDraft')}
          </Button>
          <Button size="sm" onClick={onPublish} disabled={streaming || publishing || !draft.name.trim()}>
            {publishing
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex w-[420px] shrink-0 flex-col">
          <div className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
            ...
            {streaming && (
              <div className="text-xs text-muted-foreground">{t('managed.skills.aiAuthor.thinking')}</div>
            )}
        ...
        <div className="flex flex-1 flex-col overflow-hidden bg-muted/20 px-4 pt-4 pb-3">
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="shrink-0 border-b border-border/60 px-5 py-3 text-sm font-medium text-foreground">
      {title}
    </div>
  )
}
...
function EditorSplitPane({
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      {/* Projects panel */}
      <div className="flex w-[320px] shrink-0 flex-col border-r border-border/60 bg-background">
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="w-[180px]">
          ...
          <Select
            ...
            onValueChange={(v) => onChange((p) => ({ ...p, visibility: v as SkillDraft['visibility'] }))}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
...
function ScanResultBadge({
  ...
  result: { status?: string; severity?: string | null; score?: number | null; issues_count?: number }
}) {
  const { t } = useTranslation()
  ...
  return (
    <div className={`rounded border ${color} p-3 text-xs`}>
      ...
      <div className="space-y-0.5">
        <div>
          <span className="text-muted-foreground">{t('managed.skills.aiAuthor.scan.statusLabel')}: </span>
          <span>{statusLabel}</span>
        </div>
        {result.severity && (
          <div>
            <span className="text-muted-foreground">{t('managed.skills.aiAuthor.scan.severityLabel')}: </span>
            <span>{result.severity}</span>
          </div>
        )}
        {typeof result.issues_count === 'number' && (
          <div>
            <span className="text-muted-foreground">{t('managed.skills.aiAuthor.scan.issuesLabel')}: </span>
