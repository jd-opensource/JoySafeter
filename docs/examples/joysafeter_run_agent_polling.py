#!/usr/bin/env python3
"""
JoySafeter 第三方系统调用示例：轮询 events 获取 Agent 输出。
...
环境变量：
  ...
  JOYSAFETER_POLL_INTERVAL_SEC 轮询间隔，默认 2
  JOYSAFETER_RUN_TIMEOUT_SEC   运行超时，默认 600
  JOYSAFETER_DEBUG_EVENTS      设为 1 时打印无法解析的 agent.message 原始事件
  JOYSAFETER_VERBOSE_EVENTS    设为 1 时打印所有事件/工具调用日志

运行：
  python joysafeter_run_agent_polling.py "请分析这个需求，并给出实现方案。"
"""
...
POLL_INTERVAL_SEC = float(os.getenv("JOYSAFETER_POLL_INTERVAL_SEC", "2"))
RUN_TIMEOUT_SEC = int(os.getenv("JOYSAFETER_RUN_TIMEOUT_SEC", "600"))
...
def extract_agent_message_text(event: dict[str, Any]) -> str:
    ...
    # 正常 API 会把 payload 扁平化到顶层；这里兼容极少数内部调试场景。
...
def list_events(session_id: str, after_seq: int) -> list[dict[str, Any]]:
    resp = requests.get(
...
def poll_events(session_id: str, after_seq: int = 0) -> dict[str, Any]:
    ...
    raise TimeoutError(f"session {session_id} not completed within {RUN_TIMEOUT_SEC}s")
...
def main() -> None:
    ...
    result = poll_events(session_id, after_seq=0)
