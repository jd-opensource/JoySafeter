...
> **前置要求**：已了解基本的 Agent 概念，并理解什么是 `systemPrompt` 与 `Skills`。
...
DeepAgents 让你构建的是一家真正的**公司**——一个 Manager 带领多个专业 Worker 协作完成任务。

当你在画布上创建 Agent 节点并勾选 `useDeepAgents = True` 时，后端会使用 `build_deep_agents_graph()` 将整个图构建为**动态星型拓扑（Star Topology）**。
...
1. **打开 Langfuse Dashboard**：在浏览器中访问你配置的 Langfuse 实例（如 `https://cloud.langfuse.com`），找到对应的 Trace 记录。
2. **你能看到什么**：
...
> 🚀 **全盘掌握终极预告**：至此，你已经从模型环境（01）、MCP（02）、Skills技能库连沙盒（03）、到动态管理团队（04b）全部通关实操落地。恭喜你成为 JoySafeter 真正的架构玩家！如果想用 AI 来帮你构建图？请移步**教程05：Copilot 深度解析。**
