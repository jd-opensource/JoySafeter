...
> **v1 用户注意**：v2 **没有**可视化图画布（DeepAgents）、Code 模式（浏览器写 LangGraph）与 Copilot
> 图生成器——这些已移除。v2 的 Agent 是“引擎 + 系统提示词 + 能力（工具/技能/MCP）”的声明式定义，运行在
> 一个隔离沙箱里的编码 Agent harness（Claude Code / Codex / 自研 `ccb`）中。
