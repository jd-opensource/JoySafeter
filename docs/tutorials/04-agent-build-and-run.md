...
3. orchestrator 调度器认领任务（DB 权威 + `FOR UPDATE SKIP LOCKED`），再次校验
   `Engine -> Protocol <- Provider`，再把 `SetupSandbox`（技能 / MCP / 工具 / 文件 / env）与
   `StartTask`（prompt / engine / resolved model）经 gRPC `AgentBridge` 下发给沙箱 runner。
