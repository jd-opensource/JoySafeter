...
| # | 教程 | 核心内容 | 难度 |
|---|------|---------|------|
| 01 | [模型配置：用 Secrets 管理供应商密钥](./01-model-provider-setup.md) | Secrets 页配置 Anthropic / OpenAI 兼容端点；密钥经容器 env 注入沙箱 | ⭐ 入门 |
| 02 | [为 Agent 接入 MCP 工具](./02-mcp-service-setup.md) | 在 Agent 编辑器配置 `mcp_configs`；凭据放 Vaults；运行时经 gRPC 下发 | ⭐⭐ 进阶 |
