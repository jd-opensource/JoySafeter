...
- **兼容性**: ✅ 无冲突，中断检测是后置检查，不影响正常执行；当前由 WebSocket 帧承载事件，而不是 SSE HTTP 响应

### Command 恢复流程（中断后）

#### 恢复执行（当前实现通过 WS `resume` frame）
```python
...
2. **Command 恢复与正常执行解耦**
   - 无论是历史 HTTP 端点还是当前 WS `resume` frame，本质都是在中断后向 `graph.astream_events(command, ...)` 传入 `Command`
   - 只在中断后使用，正常执行流程不涉及
...
### 1. 传输层兼容性
- ✅ LangGraph `ainvoke()` / `astream_events()` 行为不变
- ✅ 从 HTTP/SSE 切到 WS 只改变承载协议，不改变 Command 机制本身
- ✅ 当前前端事件 envelope 仍沿用原先 chat stream 的事件结构
...
**场景**: 使用历史非流式 HTTP chat 执行配置了中断的图

**行为**: `graph.ainvoke()` 会阻塞等待 Command

**解决方案**:
1. **推荐**: 需要中断的场景使用流式执行路径；当前实现统一走 `WS /ws/chat`
...
2. ✅ **恢复路径独立**: Command 恢复独立于正常消息发送路径，不影响常规执行
